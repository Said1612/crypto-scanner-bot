# -*- coding: utf-8 -*-
"""
MAFIO BOT - VERSION 16.5 (SKULL SNIPER EDITION)
السر: القوة النسبية (Relative Strength) + اقتناص العملات المنفصلة عن البتكوين
الاستراتيجية: تصفية ضجيج السوق + رصد السيولة العكسية (Anti-Market Flow)
المطور: MAFIO AI - نظام الجمجمة القناص
"""
...
# ==========================================================
# الإعدادات الاحترافية - Pro Settings (Skull Sniper Style)
# ==========================================================
...
# معايير الانفجار اللحظي (MAFIO Skull 16.5)
MIN_FLOW_RATIO = 3.8        
...
def get_btc_status():
    try:
        ...
        if data and isinstance(data, dict): return float(data.get('priceChangePercent', 0))
    except: pass
    return 0
...
def get_futures_data(sym):
    ...
    except: return {"funding": 0, "oi": 0, "status": "Unknown ⚪"}
...
def track_profits():
    ...
    ticker_dict = {t['symbol']: float(t['lastPrice']) for t in tickers if isinstance(t, dict) and 'symbol' in t}
    for sym in list(active_trades.keys()):
        if sym not in ticker_dict: continue
        current_price = ticker_dict[sym]; entry_price = active_trades[sym]['entry']; gain = ((current_price - entry_price) / entry_price) * 100
        if gain > active_trades[sym]['max_gain']: active_trades[sym]['max_gain'] = gain
        for milestone in [2, 5, 10, 15, 25, 50, 100]:
            if gain >= milestone and milestone not in active_trades[sym]['milestones']:
                active_trades[sym]['milestones'].append(milestone); duration_sec = int(time.time() - active_trades[sym]['time']); m = duration_sec // 60
                msg = (f"🔥 *{sym.replace('USDT','')} +{milestone}% milestone reached*\n"
...
def analyze_flow(sym, source):
    try:
        ...
        avg_vol = sum(vols[-11:-1]) / 10; vol_accel = vols[-1] / avg_vol if avg_vol > 0 else 1.0
        in_f = 0; out_f = 0
        for i in range(-4, 0):
        ...
        if out_f == 0: out_f = 1
        ratio = in_f / out_f; idx = -13 if len(kd) >= 13 else -len(kd); move_1h = ((closes[-1] - float(kd[idx][4])) / float(kd[idx][4])) * 100
...
def scan():
    ...
    source = state["current_source"]; logger.info(f"🔍 MAFIO Scanning {source}...")
    btc_change = get_btc_status(); market_mode = "NORMAL" if btc_change >= -0.5 else "BEARISH 🔴"
    logger.info(f"📊 Market Mode: {market_mode} (BTC: {btc_change}%)")
    tickers = get_data_from_anywhere(source, "ticker/24hr")
    if not tickers or not isinstance(tickers, list):
        new_source = "MEXC" if source == "BINANCE" else "BINANCE"; state["current_source"] = new_source; return
    candidates = []
    for t in tickers:
    ...
    for c in candidates[:40]:
        ...
        required_ratio = MIN_FLOW_RATIO; required_accel = MIN_VOL_ACCEL
        if btc_change < -1.0:
            required_ratio = MIN_FLOW_RATIO * 1.5; required_accel = MIN_VOL_ACCEL * 1.2
            if data['move_1h'] < 0.5: continue
        is_flash = data['ratio'] >= 10.0 and data['net'] >= MIN_FLASH_NET_FLOW
        is_normal = data['ratio'] >= required_ratio and data['vol_accel'] >= required_accel and data['net'] >= MIN_NET_FLOW_USD
        if is_flash or is_normal:
            ...
            if btc_change < -1.0: interest = "💀 ALPHA RELATIVE STRENGTH 💀"
            elif is_flash: interest = "⚡ FLASH LIQUIDITY SPIKE ⚡"
            elif data['vol_accel'] > 2.5: interest = "🔥 High Short Squeeze Risk 🧨"
            msg = (f"━━━━━━━━━━━━━━━━━━━━\n"
                   f"💀 *MAFIO SKULL SNIPER 16.5* 📡\n\n"
                   ...
                   "━━━━━━━━━━━━━━━━━━━━\n"
                   f"⚠️ _اقتناص الجمجمة - تم رصد قوة نسبية عكس اتجاه السوق!_ 🚀")
            send_telegram(msg); time.sleep(1)
    if state["is_first_run"]: state["is_first_run"] = False

def main():
    logger.info("🚀 MAFIO BOT 16.5 (Skull Sniper Edition) Started")
    send_telegram("💀 *MAFIO BOT 16.5* متصل.\nتم تفعيل نظام الجمجمة القناص (Skull Sniper) لاقتناص العملات المنفصلة عن السوق.")
    while True:
