...
# المعايير الصارمة (Wolf Flow Logic)
MIN_NET_FLOW = 75000         
MIN_RATIO = 5.0              
...
def get_flow_analysis(sym):
    ...
    try:
        ...
        # التأكد أن البيانات قائمة وليست رسالة خطأ نصية
...
def scan():
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"⚠️ Binance API Error: {r.status_code}", flush=True)
            return
        
        tickers = r.json()
        # هذا هو السطر الذي عالجنا فيه الخطأ الظاهر في الصورة
        if not isinstance(tickers, list):
            print("⚠️ Data format error: Expected a list of tickers", flush=True)
            return

        for t in tickers:
            # التأكد أن كل عنصر هو قاموس قبل الوصول للمفاتيح
